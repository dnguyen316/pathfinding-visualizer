  ...
  for (let row = 1; row < MAX_ROWS; row += 2) {
    for (let col = 1; col < MAX_COLS; col += 2) {
      if (row === MAX_ROWS - 2 && col === MAX_COLS - 2) {
        ...
        await destroyWall({ grid, row, col, isRight: 1, speed });
      } else if (col === MAX_COLS - 2) {
        await destroyWall({ grid, row, col, isRight: 0, speed });
      } else {
        await destroyWall({ grid, row, col, isRight: getRandInt(0, 2), speed });
      }
    }
  }
